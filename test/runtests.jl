...
@testset "Hist2D" begin
    ...
    h2 = Hist2D(Float64; binedges=(rx, ry))
    h3 = Hist2D(Float64; binedges=(rx, ry))
    push!.(h2, x, y, wgts)
...
@testset "Hist3D" begin
    ...
    h2 = Hist3D(Float64; binedges=(rx, ry, rz))
    h3 = Hist3D(Float64; binedges=(rx, ry, rz))
    push!.(h2, x, y, z, wgts)
...
@testset "Normalize with width" begin
    ...
    to = Hist1D(; binedges=[0, 1, 2, 4], overflow=true)
end
...
@testset "Bin errors" begin
    ...
    h2 = Hist1D(Float64; binedges=0:1)
    push!(h2, 0.5, 0.5)
    push!(h2, 0.5, 0.5)
...
@testset "Atomic push" begin
    ...
    a = randn(10^5)
    h1 = Hist1D(a, -3:0.2:3)
    h2 = Hist1D(Int; binedges=-3:0.2:3)
    h3 = Hist1D(Int; binedges=-3:0.2:3)
    h4 = Hist1D(Int; binedges=-3:0.2:3)
    for i in a
        push!(h2, i)
        atomic_push!(h3, i)
    end
